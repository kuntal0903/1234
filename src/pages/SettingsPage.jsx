...
import {
  ...
  ChevronRight, LogOut, Download, Cpu, X,
} from 'lucide-react';
...
function AppearanceSection({ showToast }) {
  ...
  const [density, setDensity] = useState(() => localStorage.getItem('asm_data_density') || 'comfortable');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('asm_data_density', density);
  }, [density]);
  ...
  const handleDensityChange = (d) => {
    ...
    localStorage.setItem('asm_data_density', d);
    showToast(`Data density set to ${d.toUpperCase()}`);
  };
  ...
  const handleSaveAppearance = () => {
    showToast(`Appearance updated (${theme.toUpperCase()} theme, ${density.toUpperCase()} density active)`);
  };
  ...
  return (
    <div className="settings-section" id="settings-appearance">
      ...
      <div className="settings-section__body">
        ...
        <div className="field-row">
          <div className="field-label">Data Density<span>Controls table row padding and spacing</span></div>
            ...
            {['compact', 'comfortable', 'spacious'].map(d => (
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  borderRadius: 'var(--radius-md)',
                  border: density === d ? '1px solid var(--neon-blue)' : '1px solid var(--border)',
                  background: density === d ? 'rgba(0, 242, 254, 0.12)' : 'var(--bg-card)',
                  color: density === d ? 'var(--neon-cyan)' : 'var(--text-secondary)',
                  cursor: 'pointer',
                  textTransform: 'capitalize',
                  transition: 'all 0.2s ease',
                  boxShadow: density === d ? 'var(--glow-cyan)' : 'none',
                }}
              >
                {d}
              </button>
